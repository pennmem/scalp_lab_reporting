...
def calculate_bonus_repfr(subj):
    """
    ...
    for sess in range(n_sessions):
        ...
        event_file = '/protocols/ltp/subjects/%s/experiments/ltpRepFR/sessions/%d/behavioral/current_processed/task_events.json' % (subj, sess)
        prec = np.nan
        lbr = np.nan
        rbr = np.nan
        br = np.nan
        try:
            ...
            sess_dir = '/data/eeg/scalp/ltp/ltpRepFR/%s/session_%d/' % (subj, sess)
            sess_precs = []
