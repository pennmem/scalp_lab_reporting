...
def bonus_report_delayrepfr(subj, scores, bonuses, nans_blank=True):
    """
    ...
    outfile = '/data/eeg/scalp/ltp/ltpDelayRepFRReadOnly/bonus/%s_bonus_report.tsv' % subj
    ...
    if subj not in ('LTP356', 'LTP392', 'LTP400', 'LTP400', 'LTP401', 'LTP402', 'LTP404'):
        ...
        for i in range(len(bonuses)):
            report += '%d\t%.1f%%\t$%.2f\t%s\t$%.2f\t$%.2f\n' % \
                      (i,
                       ...
                       str(scores[i][1]) + '%/' + str(scores[i][2]) + '%/' + str(scores[i][3]) + '%', bonuses[i][1], bonuses[i][2])
        ...
        report += 'Overall\t%.1f%%\t$%.2f\t%s\t$%.2f\t$%.2f\n' % \
                  (np.nanmean([s[0] for s in scores]), np.nansum([b[0] for b in bonuses]),
                   ...
                   str(round(np.nanmean([s[4] for s in scores]), 1)) + '%/' +
                   str(round(np.nanmean([s[5] for s in scores]), 1)) + '%', np.nansum([b[1] for b in bonuses]),
    ...
    tex_outfile = '/data/eeg/scalp/ltp/ltpDelayRepFRReadOnly/bonus/%s_bonus_report' % subj
