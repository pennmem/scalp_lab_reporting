...
def make_recalls_matrix(pres_nos, rec_nos):
    ...
    """
    recalls = np.zeros_like(rec_nos, dtype='int16')
...
def make_data_matrices_prelim(subj):
    """
    ...
    for i in range(n_sess):
        ...
        if os.path.exists(os.path.join(sess_path, 'session.jsonl')):
    ...
    wordpool = np.loadtxt(os.path.join(exp_dir, 'RAM_wordpool.txt'), dtype='S32')
    ...
    for sess_num, session_dir in enumerate(sess_dirs):
        ...
        sess_recalls = make_recalls_matrix(sess_pres_nos, sess_rec_nos)
        ...
        if sess_recalls is None:
            ...
            mat_pairs.append((recalls, sess_recalls))
