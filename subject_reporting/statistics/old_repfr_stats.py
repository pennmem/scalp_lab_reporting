...
def run_stats_repfr(subj, data=None):
    """
    Runs a selection of statistics on the data from one RepFR participant. Stats are run separately for each of that
    participant's sessions, and are returned in a dictionary mapping each stat's name to its results. The stats that
    are run for RepFR participants are as follows:

    - Total correct recalls (recalls)
    - Total intrusions (intrusions)
    - Total repetitions (repetitions) -- Note that this counts the number of repetitions of correct words; repeated ELIs
        contribute instead to the ELI count
    ...
    for lst, lst_data in enumerate(recs):
        if np.all(np.isnan(recalled[lst, :])):
            stats['recalls'][lst] = np.nan
            stats['intrusions'][lst] = np.nan
            stats['repetitions'][lst] = np.nan
        else:
            for i, w in enumerate(lst_data):
                if w != '' and np.isin(w, lst_data[:i]) and intru[lst, i] == 0:
                    stats['repetitions'][lst // len(stats['session'])] += 1
