...
def run_stats_repfr(subj, data=None):
    """
    Runs a selection of statistics on the data from one ltpRepFR participant. Stats are run separately for each of that
    participant's sessions, and are returned in a dictionary mapping each stat's name to its results. The stats that
    are run for ltpRepFR participants are as follows:
    ...
    :param data: Optional. A dictionary containing the behavioral matrices generated by behavioral_matrices_ltpRepFR. If
    None, loads the data from /data/eeg/scalp/ltp/ltpRepFR/behavioral/data/beh_data_LTP*.json.
...
#    stats['crl'] = crl(spos, times, sessions, ll, lag_num=ll - 1)
    ...
    fig_dir = '/data/eeg/scalp/ltp/ltpRepFR/%s/figs/' % subj
    ...
    for i, sess in enumerate(stats['session']):
        fig_dir = '/data/eeg/scalp/ltp/ltpRepFR/%s/session_%d/figs/' % (subj, sess)
